...
export default function FBOps() {
  ...
  const handleBookEvent = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Pré-pagamento processado via gateway.',
      })
  ...
  const handleDynamicPrice = () => {
    toast({
      title: 'Motor Acionado',
      description: 'Preços atualizados com base na demanda (+15%).',
    })
  }
    ...
    <div className="space-y-6 animate-fade-in pb-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Eventos gastronómicos, precificação dinâmica e logística de salão.
        </p>
      </div>
      ...
      <Tabs defaultValue="eventos" className="w-full">
        ...
        <TabsContent value="eventos" className="grid gap-6 md:grid-cols-2">
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <CalendarClock className="w-5 h-5" /> Calendário de Eventos
              </CardTitle>
            </CardHeader>
              ...
              {events.map((ev) => (
                <div key={ev.id} className="p-4 border rounded-lg bg-slate-50">
                  ...
                  <div className="mt-3 font-bold text-emerald-700">
                    {formatCurrency(ev.price, ev.currency)} / pax
                  </div>
                </div>
              ))}
            </CardContent>
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle>Reservar & Pré-pagamento</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="1"
                  value={bookData.pax}
                  onChange={(e) => setBookData({ ...bookData, pax: parseInt(e.target.value) })}
                />
              </div>
              <Button onClick={handleBookEvent} className="w-full">
                Processar Reserva
              </Button>
            </CardContent>
          </Card>
        ...
        <TabsContent value="dinamico">
          <Card className="shadow-sm max-w-xl">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <TrendingUp className="w-5 h-5" /> Pricing Engine
              </CardTitle>
              <CardDescription>Ajuste preços baseado em tags de demanda.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-md text-amber-800 text-sm">
                <strong>Alerta de Demanda:</strong> Alta ocupação detectada para o fim de semana.
                Sugere-se aplicar tag "Sazonalidade Alta".
              </div>
              <Button onClick={handleDynamicPrice} className="w-full bg-slate-900">
                Aplicar Atualização em Lote (+15%)
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="rotas">
          <Card className="shadow-sm max-w-xl">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Route className="w-5 h-5" /> Otimização de Salão
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="p-3 border-l-4 border-blue-500 bg-slate-50 text-sm">
                  <strong>Garçom 1 (João):</strong> Rota Otimizada {'->'} Mesa 12 (Entrega) {'->'}{' '}
                  Mesa 14 (Recolha) {'->'} Cozinha
                </div>
                <div className="p-3 border-l-4 border-emerald-500 bg-slate-50 text-sm">
                  <strong>Garçom 2 (Ana):</strong> Rota Otimizada {'->'} Bar {'->'} Mesa 05
                  (Bebidas) {'->'} Receber Mesa 02
                </div>
              </div>
              <p className="text-xs text-slate-500 mt-4">
                * Rotas calculadas em tempo real baseadas no KDS.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
